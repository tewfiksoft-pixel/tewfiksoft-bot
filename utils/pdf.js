  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      const drawStamp = (x, y, title, name, color) => {
        ...
        doc.font(fontNormal).fontSize(6).fillColor('#555').text('Sign� �lectroniquement:', x + 5, y + 15);
        doc.font(fontBold).fontSize(7.5).fillColor(color).text(name, x, y + 23, { width: 110, align: 'center' });
        doc.font('Helvetica-Oblique').fontSize(5).fillColor(color).text('DOCUMENT VALID�', x, y + 34, { width: 110, align: 'center' });
      };
      ...
      doc.font(fontNormal).text('VISA ET CACHET:', startX + 555, insideY + 4);
      drawStamp(startX + 560, insideY + 14, 'EXP�DITION GDS', data.gdsName, '#e3a21a');
      ...
      drawStamp(startX + 560, insideY - 14 + 26, 'COMPTABILIT�', data.financeName, '#00a300');
