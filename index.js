...
const canViewEmployees = (role) => role !== 'general_manager';
...
const handleStart = async (chatId, user, lang) => {
    ...
    const msg = ar ? 
          ...
          "Bienvenue " + (user.name || '') + "!\\n------------------\\nID: " + user.id + "\\nRole: " + (roleLabel[user.role] || user.role) + "\\n\\nCommandes:\\n/me - Votre carte\\n/info - Rechercher un employe\\n/menu - Menu principal";
    await send(chatId, msg);
};

const handleMe = async (chatId, user, lang) => {
    const ar = lang === 'ar';
    const msg = ar ? "Card d'Identite\\n------------------\\nID: " + user.id + "\\nNom: " + (user.name || '-') + "\\nRole: " + user.role : "Carte d'identite\\n------------------\\nID: " + user.id + "\\nNom: " + (user.name || '-') + "\\nRole: " + user.role;
    await send(chatId, msg);
};
...
const handleMenu = async (chatId, user, lang) => {
    ...
    if (!isEmployee(user.role)) {
          kbd.inline_keyboard.push([{ text: ar ? 'Search Employee' : 'Rechercher employe', callback_data: 'emp_search' }]);
          kbd.inline_keyboard.push([{ text: ar ? 'Request Document' : 'Demander document', callback_data: 'show_docs' }]);
    }
    if (canViewStats(user.role)) kbd.inline_keyboard.push([{ text: ar ? 'Statistics' : 'Statistiques', callback_data: 'stats' }]);
    if (user.role === 'admin') kbd.inline_keyboard.push([{ text: ar ? 'Sync Database' : 'Sync base', callback_data: 'sync_db' }]);
    kbd.inline_keyboard.push([{ text: ar ? 'Change Language' : 'Changer langue', callback_data: 'choose_lang' }]);
    await send(chatId, ar ? 'Main Menu' : 'Menu Principal', kbd);
};
...
const handleInfoSearch = async (chatId, user, lang, query) => {
    ...
    if (results.length === 0) {
          return send(chatId, ar ? "Employee not found." : "Aucun employe trouve.");
    }
...
const handleInfo = async (chatId, user, lang) => {
    if (!canViewEmployees(user.role)) {
          const ar = lang === 'ar';
          return send(chatId, ar ? 'Role error.' : 'Votre role permet uniquement les statistiques.');
    }
    const ar = lang === 'ar';
    states.set(String(chatId), { step: 'search_query' });
    await send(chatId, ar ? 'Enter Name or ID:' : 'Nom ou matricule de l\'employe:');
};
...
const handleDocRequest = async (chatId, user, lang, config) => {
    ...
    if (!docs.length) return send(chatId, ar ? 'No docs available.' : 'Aucun document disponible.');
    const kbd = { inline_keyboard: docs.map(d => [{ text: ar ? d.ar : d.fr, callback_data: "doc:" + d.id }]) };
    await send(chatId, ar ? 'Choose Document:' : 'Choisissez le document:', kbd);
};
...
const handleMotifs = async (chatId, lang) => {
    ...
    await send(chatId, ar ? 'What is the motif?' : 'Quel est l\'usage du document?', kbd);
};

const handleConfirmation = async (chatId, lang) => {
    const ar = lang === 'ar';
    await send(chatId, ar ? 'Request Received.\\nAdministration will contact you soon.' : 'Demande recue.\\nL\'administration etudiera votre demande.');
};
...
const handle = async (update, config) => {
    ...
    if (!user) {
          if (txtL === '/start' || txtL === '/me' || txtL === '/id') {
                  return send(chatId, "Acces Refuse\\n------------------\\nID: " + fromId + "\\n\\nContact Admin.");
          }
          return;
    }
    ...
    if (cbq) {
          ...
          if (data === 'sync_db') {
                  if (user.role !== 'admin') return send(chatId, "No Permission.");
                  await send(chatId, "Syncing...");
                  await syncFromDrive();
                  return send(chatId, "Sync Complete.");
          }
          if (data === 'stats') {
                  if (!canViewStats(user.role)) return send(chatId, ar ? 'No Permission.' : 'Permission refusee.');
                  return send(chatId, ar ? 'Statistics\\n------------------\\nUse Admin Panel.' : 'Statistiques\\n------------------\\nTableau de bord.');
          }
          if (data.startsWith('doc:')) {
                  ...
                  return send(chatId, ar ? 'Motif?' : 'Motif de la demande?');
          }
    ...
    if (txtL.startsWith('/lang')) {
          const kbd = { inline_keyboard: [[{ text: 'Arabic', callback_data: 'lang:ar' }, { text: 'Francais', callback_data: 'lang:fr' }]] };
          return send(chatId, 'Language / Langue', kbd);
    }
};
